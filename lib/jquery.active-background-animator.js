...
(function( $ ) {
	...
	$.fn.activeBackgroundAnimator = function(layers,options) {
		// options
		var settings = $.extend({
			test: "yay"
		}, options );
		...
		function ABA(container,layers,settings){
			...
			this.animation = window.ABA.ActiveGraph();
			
			this.animation.init(this.canvas);
		...
		//if(layers && layers.constructor === Array && layers.length>0){
			this.each(function() {
				new ABA($(this),layers,settings);
			});
		//}
	...
	return new function(){
			this.init = function(canvas){
				...
				for (var i = 0; i < this.nb; i++){
					this.nodes.push(new Node(this,new ABA.Position(ABA.randomIntFromInterval(0,this.canvas.getWidth()),
															ABA.randomIntFromInterval(0,this.canvas.getHeight()))));
				}
			}
