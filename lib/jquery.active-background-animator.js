...
(function( $ ) {
	...
	$.fn.activeBackgroundAnimator = function(layers,options) {
		...
		if(layers){
			...
			this.each(function() {
				new Animator($(this),layers,settings).init();
			});
		}
...
var ABA = {
		...
		getAnimation(name){
			return new ABA._animations[name];
		},
...
ABA.registerAnimation("ActiveGraph",function(){
	this.init = function(canvas){
		...
		this.nb = Math.min(this.canvas.surface*3,140);
		this.near = 2;
